...
class ChannelShuffle(nn.Module):
    ...
    def forward(self, x):
        n, c, d, h, w = x.shape
        x = x.view(n, self.groups, c // self.groups, d, h, w)  # Group
        x = x.transpose(1, 2).contiguous().view(n, -1, d, h, w)  # Shuffle
        return x
...
class ResidualBottleneck(nn.Module):
    '''
    ShuffleNet V2 basic unit (adapted for 3D).
    '''
...
class ResidualDownsample(nn.Module):
    '''
    ShuffleNet V2 unit for spatial down-sampling (adapted for 3D).
    '''
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.branch1 = nn.Sequential(
            nn.Conv3d(in_channels, in_channels, 3, 2, 1, groups=in_channels),
...
class TimeMLP(nn.Module):
    '''
    Introduce timestep information to feature maps with MLP and add shortcut (adapted for 3D).
    '''
